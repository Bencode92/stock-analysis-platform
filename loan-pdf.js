 ...
 *   • 📋 Saut forcé avant Comparaison si PTZ activé
 *   • 📋 Saut systématique entre Comparaison et Équivalence  
 *   • 🔒 Protection page-break-inside: avoid !important
 *
...
export async function exportLoanToPDF(loanData = null, options = {}) {
  ...
  try {
    ...
    if (isDev) console.log('✅ PDF v2.6.2 généré - tableaux coupés corrigés');
  } catch (err) {
...
function extractLoanDataFromDOM() {
  ...
  if (isDev) {
    ...
    if (data.comparisonHtml) console.log('🔧 Tableau Comparaison extrait et nettoyé v2.6.2');
    if (data.equivalenceHtml) console.log('🔧 Tableau Équivalence extrait et fixé v2.6.2');
...
// ──────────────────────────────
// 1. Styles CSS v2.6.2 - Refonte esthétique complète + fix tableaux
// ──────────────────────────────
function buildStyles(){
  ...
  s.textContent=`
    ...
    /* 🔧 #18: Styles tableaux v2.6.2 - overflow complet neutralisé + protection coupure */
    .analysis-block{
      margin:6mm 0; /* 📏 Marge harmonisée */
      padding:5mm;
      ...
      page-break-inside:avoid !important; /* 🔒 bloc entier */
...
export function activateLoanExportButton(){
  ...
  if(btn){
    ...
    btn.title='Télécharger la synthèse PDF v2.6.2 avec tableaux non coupés';
