 ...
 * 
 * OPTIMISATIONS v2.0:
 * - Cash-flows après impôt dans les projections
 * - IRA corrigée (3% au lieu de 2%)
 * - Paramètres ajustables (frais revente, part terrain, taux IS)
 * - Flag réintégration LMNP 2025
 */
...
const ImmoExtensions = (function() {
    ...
    const PARAMETRES_REVENTE = {
        fraisReventePct: 7,  // Frais de revente par défaut (7% au lieu de 10%)
        partTerrain: 20,     // Part du terrain par défaut (20%)
        eligibleTauxReduitIS: false,  // Éligibilité au taux réduit IS
        reintegrationAmortLMNP2025: true  // Réintégration des amortissements LMNP
    };
    ...
    function calculerImpotPlusValueSelonRegime({
        ...
        partTerrain = null,  // NOUVEAU: paramètre optionnel
        reintegrationAmortLMNP = null  // NOUVEAU: paramètre optionnel
    }) {
        // Utiliser les valeurs par défaut si non fournies
        partTerrain = partTerrain !== null ? partTerrain : PARAMETRES_REVENTE.partTerrain;
        reintegrationAmortLMNP = reintegrationAmortLMNP !== null ? reintegrationAmortLMNP : PARAMETRES_REVENTE.reintegrationAmortLMNP2025;
        ...
        if (['micro-foncier','reel-foncier','lmnp-micro','lmnp-reel'].includes(regime)) {
            ...
            if (regime === 'lmnp-reel' && reintegrationAmortLMNP) {
        ...
        if (regime.endsWith('-is')) {
            // Valeur comptable : terrain avec part paramétrable
            ...
            // Barème IS optimisé avec éligibilité au taux réduit
            const eligibleTauxReduit = PARAMETRES_REVENTE.eligibleTauxReduitIS;
            const part15 = eligibleTauxReduit ? Math.min(baseIS, 42500) : 0;
            const impotIS = part15 * 0.15 + Math.max(0, baseIS - part15) * (tauxIS / 100);
    ...
    function initialiser(simulateurInstance) {
        ...
        // NOUVEAU: Initialiser les paramètres de revente dans le simulateur
        if (!simulateur.params.revente) {
            simulateur.params.revente = { ...PARAMETRES_REVENTE };
        }
    ...
    function etendreSimulateur() {
        ...
        SimulateurImmo.prototype.calculerImpactFiscalAvecRegime = function(revenuFoncier, interetsEmprunt, charges, regimeFiscal) {
            ...
            switch (regimeFiscal) {
                ...
                case 'sarl-is':
                    ...
                    // Calcul de l'IS avec taux réduit conditionnel
                    const eligibleTauxReduit = this.params.revente?.eligibleTauxReduitIS || false;
                    const part15 = eligibleTauxReduit ? Math.min(Math.max(0, resultatIS), 42500) : 0;
                    const tauxPlein = this.params.fiscalite.tauxIS / 100 || 0.25;
                    const impotIS = part15 * 0.15 + Math.max(0, resultatIS - part15) * tauxPlein;
                    ...
                    return {
                        ...
                        tauxIS: tauxPlein * 100,
                        eligibleTauxReduit: eligibleTauxReduit
                    };
        ...
        SimulateurImmo.prototype.calculerScenarioRevente = function(investissement, nbAnnees, tauxAppreciationAnnuel) {
            ...
            const impactFiscalAnnuel = Number(investissement.impactFiscal) || 0; // signe + = gain fiscal
            const cashFlowApresImpotsMensuel = cashFlowAvantImpotsMensuel + impactFiscalAnnuel / 12;
            const cashFlowAnnuel = cashFlowApresImpotsMensuel * 12; // <-- Utilise maintenant les CF après impôt
            ...
            // OPTIMISATION: Frais de revente paramétrables (défaut 7%)
            const tauxFraisRevente = this.params.revente?.fraisReventePct || 7;
            const fraisRevente = valeurRevente * (tauxFraisRevente/100);
            
            // Calcul plus-value via helper 2025 avec paramètres optimisés
            ...
            const resPV = calculerImpotPlusValueSelonRegime({
                ...
                amortissementAnnuel,
                tauxIS,
                partTerrain: this.params.revente?.partTerrain,
                reintegrationAmortLMNP: this.params.revente?.reintegrationAmortLMNP2025
            });
            ...
            // OPTIMISATION 2: IRA corrigée (3% au lieu de 2%)
            const tauxMensuel = (this.params.base.taux / 100) / 12;
            const iraFrance = Math.min(
                capitalRestantDu * 0.03,        // 3% du CRD (au lieu de 2%)
                capitalRestantDu * tauxMensuel * 6  // 6 mois d'intérêts
            );
            
            // Cash-flows cumulés (maintenant après impôt)
            const cashFlowsCumules = cashFlowAnnuel * nbAnnees;
            
            // Valeur nette après revente
            const valeurNetteRevente = valeurRevente - fraisRevente - impotPlusValueTotal - capitalRestantDu - iraFrance;
            ...
            // Calcul du TRI avec cash-flows après impôt
            const fluxTresorerie = [];
            fluxTresorerie.push(-apportInitial);
            
            for (let i = 1; i < nbAnnees; i++) {
                fluxTresorerie.push(cashFlowAnnuel); // Déjà après impôt maintenant
            }
            ...
            // OPTIMISATION 3: Protection contre les valeurs négatives
            const plusValueBruteSansMajoration = Math.max(0, valeurRevente - prixAchat);
            ...
            return {
                ...
                plusValueBrute,
                plusValueBruteSansMajoration,
                fraisRevente,
                fraisReventePct: tauxFraisRevente,
                impotPlusValue: {
                    ir: impotPlusValueIR,
                    ps: impotPlusValuePS,
                    surtaxe: surtaxe,
                ...
                capitalRestantDu,
                fraRemboursementAnticipe: iraFrance, // Valeur corrigée
                valeurNetteRevente,
                cashFlowsCumules,
                cashFlowApresImpot: cashFlowAnnuel / 12,
                gainTotal,
        ...
        // ===============================================================
        // MÉTHODE CALCULER AMORTISSEMENT ANNUEL - VERSION OPTIMISÉE
        // ===============================================================
        SimulateurImmo.prototype.calculerAmortissementAnnuel = function(regime) {
            if (!['lmnp-reel', 'lmp-reel', 'sci-is', 'sas-is', 'sarl-is'].includes(regime)) return 0;
            ...
            // Part du terrain paramétrable
            const partTerrain = (this.params.revente?.partTerrain || 20) / 100;
        ...
        // ===============================================================
        SimulateurImmo.prototype.grilleSensibilite = function(investissement, annees, 
                                                                appVals = [0, 1, 2, 3], 
                                                                fraisPcts = [5, 7, 10]) {
            const resultats = [];
            
            // Sauvegarder les valeurs actuelles
            const fraisReventeOriginal = this.params.revente?.fraisReventePct || 7;
            
            for (const app of appVals) {
                for (const frais of fraisPcts) {
                    // Définir temporairement les frais de revente
                    if (this.params.revente) {
                        this.params.revente.fraisReventePct = frais;
                    }
                    
                    const scenario = this.calculerScenarioRevente(investissement, annees, app);
                    ...
                    resultats.push({
                        ...
                        cashFlowsCumules: scenario.cashFlowsCumules,
                        valeurNetteRevente: scenario.valeurNetteRevente
                    });
                }
            }
            ...
            if (this.params.revente) {
                this.params.revente.fraisReventePct = fraisReventeOriginal;
            }
    ...
    function ajouterStyles() {
        const styles = `
            ...
            /* Styles pour les nouveaux paramètres */
            .params-revente {
    ...
    function ajouterElementsInterface() {
        ...
        // 3. NOUVEAU: Ajouter les paramètres de revente
        ajouterParametresRevente();
    }
...
function ajouterSelectionRegimeFiscal() {
        ...
        sectionFiscale.innerHTML = `
            ...
            <div class="card-body p-4">
                ...
                <div id="taux-is-container" style="display: none;">
                    ...
                    <!-- NOUVEAU: Checkbox pour le taux réduit -->
                    <div class="mt-2">
                        <label class="flex items-center">
                            <input type="checkbox" id="eligible-taux-reduit-is" class="mr-2">
                            <span>Éligible au taux réduit IS (15% jusqu'à 42 500€)</span>
                        </label>
                        <span class="form-help">Cochez si votre société remplit les conditions du taux réduit</span>
                    </div>
                </div>
    ...
    // NOUVELLE FONCTION: Ajouter les paramètres de revente
    function ajouterParametresRevente() {
        const scenariosCard = document.getElementById('scenarios-card');
        if (!scenariosCard) return;
        
        // Chercher la grille existante
        const gridContainer = scenariosCard.querySelector('.grid.grid-2');
        if (!gridContainer) return;
        
        // Créer un nouveau conteneur pour les paramètres avancés
        const paramsContainer = document.createElement('div');
        paramsContainer.className = 'params-revente';
        paramsContainer.innerHTML = `
            <h4><i class="fas fa-cog mr-2"></i>Paramètres de revente</h4>
            <div class="grid grid-3">
                <div class="form-group">
                    <label class="form-label">Frais de revente</label>
                    <div class="form-addon">
                        <input type="number" id="frais-revente-pct" class="form-input" value="7" min="3" max="15" step="0.5">
                        <span class="form-addon-text">%</span>
                    </div>
                    <span class="form-help">Agence, diagnostics, rafraîchissement</span>
                </div>
                
                <div class="form-group">
                    <label class="form-label">Part du terrain</label>
                    <div class="form-addon">
                        <input type="number" id="part-terrain" class="form-input" value="20" min="10" max="35" step="5">
                        <span class="form-addon-text">%</span>
                    </div>
                    <span class="form-help">Non amortissable</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div>
                        <label class="flex items-center mb-2">
                            <input type="checkbox" id="reintegration-lmnp-2025" checked class="mr-2">
                            <span>Réintégration LMNP 2025</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        // Insérer après la grille existante
        gridContainer.parentNode.insertBefore(paramsContainer, gridContainer.nextSibling);
        
        // Ajouter les écouteurs
        document.getElementById('frais-revente-pct')?.addEventListener('change', (e) => {
            if (simulateur && simulateur.params.revente) {
                simulateur.params.revente.fraisReventePct = parseFloat(e.target.value) || 7;
            }
        });
        
        document.getElementById('part-terrain')?.addEventListener('change', (e) => {
            if (simulateur && simulateur.params.revente) {
                simulateur.params.revente.partTerrain = parseFloat(e.target.value) || 20;
            }
        });
        
        document.getElementById('reintegration-lmnp-2025')?.addEventListener('change', (e) => {
            if (simulateur && simulateur.params.revente) {
                simulateur.params.revente.reintegrationAmortLMNP2025 = e.target.checked;
            }
        });
    }
    ...
    function initialiserEvenementsRegimeCards() {
        ...
        const eligibleCheckbox = document.getElementById('eligible-taux-reduit-is');
        ...
        // NOUVEAU: Écouteur pour le checkbox taux réduit
        if (eligibleCheckbox) {
            eligibleCheckbox.addEventListener('change', (e) => {
                if (simulateur && simulateur.params.revente) {
                    simulateur.params.revente.eligibleTauxReduitIS = e.target.checked;
                }
            });
        }
                    ...
                    <div class="results-card">
                        ...
                        <div class="results-body">
                            <table class="comparison-table">
                                ...
                                <tr>
                                    <td>Indemnités remb. anticipé (IRA)</td>
                                    <td>${formaterMontant(res.fraRemboursementAnticipe)}</td>
                                </tr>
                                <tr class="border-t border-gray-600">
                                    <td>Cash-flows après impôt/mois</td>
                                    <td class="${res.cashFlowApresImpot >= 0 ? 'positive' : 'negative'}">${formaterMontantMensuel(res.cashFlowApresImpot)}</td>
                                </tr>
                                <tr>
            ...
            <div class="info-message mt-4">
                ...
                <div>
                    ...
                    <p class="text-sm opacity-90">
                        ✅ Cash-flows après impôt utilisés dans le calcul du TRI<br>
                        ✅ IRA corrigée : min(3% du CRD, 6 mois d'intérêts)<br>
                        ✅ Frais de revente paramétrables (${resultatsClassique.fraisReventePct || 7}%)<br>
                        ${resultatsClassique.detailPlusValue?.detail?.partTerrain ? 
                            `✅ Part du terrain : ${resultatsClassique.detailPlusValue.detail.partTerrain}%<br>` : ''}
                        ${resultatsClassique.regime === 'lmnp-reel' ? 
                            `${simulateur.params.revente?.reintegrationAmortLMNP2025 ? 
                                '⚠️ LMNP réel : réintégration des amortissements activée' : 
                                '✅ LMNP réel : réintégration des amortissements désactivée'}<br>` : ''}
                        ${resultatsClassique.regime.endsWith('-is') ? 
                            `Régime IS : ${simulateur.params.revente?.eligibleTauxReduitIS ? 
                                'taux réduit 15% jusqu\'à 42.5k€' : 'taux plein ' + (simulateur.params.fiscalite.tauxIS || 25) + '%'}<br>` : ''}
                        Abattements selon durée de détention (exonération IR à 22 ans, PS à 30 ans)
                    </p>
                </div>
            </div>
        `;
    ...
    function mettreAJourElementsFiscauxParMode(mode, resultats, regimeLabel) {
        ...
        const fiscal = {
            ...
            eligibleTauxReduit: resultats.fiscalDetail.eligibleTauxReduit || false
        };
        ...
        if (fiscal.type === 'IS') {
            explanation = `En régime d'imposition à l'IS (Impôt sur les Sociétés), le résultat fiscal de la société est taxé directement 
                au taux d'IS de ${fiscal.tauxIS}%. ${fiscal.eligibleTauxReduit ? 'Taux réduit 15% appliqué jusqu\'à 42.5k€.' : ''} Les amortissements sont déductibles, ce qui réduit l'assiette fiscale.`;
        } else {
        ...
        if (!fiscalInfo) {
            ...
            fiscalDiv.innerHTML = `
                ...
                <table class="comparison-table">
                    ...
                    <tr>
                        <td>${fiscal.type === 'IS' ? `Impôt société (${fiscal.eligibleTauxReduit ? '15%/25%' : fiscal.tauxIS + '%'})` : 'Impact fiscal annuel'}</td>
