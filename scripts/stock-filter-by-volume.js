...
// ✅ Spec elite ROIC 6 ans §3 clé 2 : persistance = nb d'exercices avec ratio ≥ seuil.
function arrPersist(arr, threshold) {
  const v = arr.filter(Number.isFinite);
  if (!v.length) return null;
  return v.filter(x => x >= threshold).length;
}

// ✅ Spec elite ROIC 6 ans §3 clé 3 : semi-déviation SOUS la médiane uniquement.
// Une hausse ne pénalise pas ; seule la baisse compte (interdit : écart-type symétrique).
function arrDownsideDev(arr) {
  const v = arr.filter(Number.isFinite);
  if (v.length < 2) return null;
  const med = arrMedian(v);
  const below = v.filter(x => x < med).map(x => (x - med) ** 2);
  if (!below.length) return 0;                       // jamais sous la médiane = 0 (idéal)
  return Math.sqrt(below.reduce((a, b) => a + b, 0) / below.length);
}

// ✅ T2 (spec §8) : MAX DRAWDOWN du ROIC = plus grande baisse pic → creux ULTÉRIEUR, en % du pic.
// Un riser pur = 0 (une hausse ne pénalise pas) ; une vraie chute pénalise. Remplace la semi-déviation.
// L'entrée yearly_* est en ordre récent→ancien → on remet en chronologique (ancien→récent) d'abord.
function arrMaxDrawdown(arr) {
  const v = arr.filter(Number.isFinite);
  if (v.length < 2) return null;
  const chrono = [...v].reverse();
...
function deriveElite6y(src) {
  const r6 = (src.yearly_roic || []).filter(Number.isFinite);
  const e6 = (src.yearly_roe || []).filter(Number.isFinite);
  const rd = arrDownsideDev(r6), ed = arrDownsideDev(e6);
  return {
...
async function fetchBalanceSheet(symbol, context = {}) {
  try {
    ...
    if (error.response?.status === 429) {
      console.warn(`  ⏱️ Rate limit (429) balance_sheet ${symbol}`);
...
async function fetchIncomeStatement(symbol, context = {}) {
  try {
    ...
    // ✅ HTTP 429 = rate limit -> déclenche le filet pause+retry (au lieu d'abandonner le titre)
    if (error.response?.status === 429) {
      console.warn(`  ⏱️ Rate limit (429) income_statement ${symbol}`);
      return { _rateLimited: true };
    }
...
function computeMultiYearRatios(bsPeriods, isPeriods) {
  ...
  // ✅ Spec elite ROIC 6 ans (docs/ELITE_ROIC_10Y_SPEC.md §3) : clés de départage figées.
  // Base = série ROIC/ROE par exercice DÉJÀ calculée (yearlyRatios, jusqu'à 6 ans),
  // pas de refetch. Seuil 12 % = porte rentabilité elite ; equity_elite choisit ROIC
  // (non-fin.) ou ROE (financières). years_available < 4 → « historique court » côté ranker.
  ...
  const roic_drawdown_6y = arrMaxDrawdown(roic6);      // ✅ T2 : max drawdown ROIC (remplace la semi-déviation)
  const roe_drawdown_6y  = arrMaxDrawdown(roe6);
...
async function enrichWithFundamentals(stocks, maxNewFetches = MAX_NEW_FETCHES_PER_RUN) {
  ...
  for (const stock of stocks) {
    ...
    if (cached && cached.fetched_at) {
      ...
      if (cacheNotExpired && formulaOk && !needsContextMigration && !isEmptyData) {
        ...
        stock.roic_drawdown_6y = _e6.roic_drawdown_6y ?? null;
        stock.roe_drawdown_6y = _e6.roe_drawdown_6y ?? null;
        stock.years_roic_6y = _e6.years_roic_6y ?? null;
  ...
  for (const stock of toProcess) {
    ...
    try {
      ...
      stock.roic_drawdown_6y = fundamentals.roic_drawdown_6y ?? null;
      stock.roe_drawdown_6y = fundamentals.roe_drawdown_6y ?? null;
      stock.years_roic_6y = fundamentals.years_roic_6y ?? null;
  ...
  for (const stock of notProcessed) {
    ...
    stock.roic_drawdown_6y = _e6b.roic_drawdown_6y ?? null;
    stock.roe_drawdown_6y = _e6b.roe_drawdown_6y ?? null;
    stock.years_roic_6y = _e6b.years_roic_6y ?? null;
    stock.net_margin = cached?.net_margin ?? null;
    stock.revenue_growth_3y = cached?.revenue_growth_3y ?? null;
  }
