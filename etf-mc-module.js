...
(function () {
  // ---------- utils ----------
  const waitFor = (cond,cb,tries=40)=>cond()?cb():tries<=0?console.error('❌ ETF MC: données introuvables'):setTimeout(()=>waitFor(cond,cb,tries-1),250);
  ...
  function init(){
    ...
    if(!document.getElementById('etf-mc-v37-styles')){
      ...
      .facet-list{ list-style:none; margin:6px 0 0; padding:6px 8px; background:rgba(0,160,255,.07); border:1px solid rgba(0,160,255,.18); border-radius:8px }
      ...
      /* masque les vieux champs TER/AUM s'ils existent */
      #etf-filter-ter, #etf-filter-aum{ display:none !important } 
      label[for="etf-filter-ter"], label[for="etf-filter-aum"]{ display:none !important }
      /* Pills métriques */
    ...
    const state = {
      mode:'balanced',
      selectedMetrics:['return_ytd','dividend_yield','volatility','sharpe_proxy','aum','ter','return_1y','return_1d'],
      filters:{ countries:new Set(), sectors:new Set(), fundTypes:new Set(), excludeLeveraged:true }, // TER/AUM supprimés
    ...
    function buildFacetCatalogs(){
      ...
      state.catalogs.countries=sort(cCount);
      state.catalogs.sectors=sort(sCount);
      state.catalogs.fundTypes=sort(fCount);
    }

    // ---------- collapsed facet lists (comme Actions) ----------
    function makeFacetLists(){
      const host = root.querySelector('fieldset:last-of-type'); if(!host) return;

      // cache/masque les vieux selects (et TER/AUM)
      ...
      const mk = (id,title,values,facet,dict)=>{
        ...
        wrap.innerHTML=`
          <div class="facet-head"><span>▸</span><span>${title}</span><span class="count">(0)</span></div>
          <div class="facet-body">
            <ul id="${id}" class="facet-list">
        ...
        wrap.querySelectorAll('input[data-facet]').forEach(inp=>{
          const sets=state.filters;
          const target = facet==='country' ? sets.countries : facet==='sector' ? sets.sectors : sets.fundTypes;
    ...
    function updateCount(groupEl, n){ groupEl.querySelector('.count').textContent = `(${n})`; }
    ...
    function setupCustomFiltersUI(){
      ...
      const updUnit=()=>unitSpan.textContent = METRICS[metricSel.value]?.unit || '';
      updUnit(); metricSel.addEventListener('change', updUnit);
      ...
      function renderList(){
        ...
        listBox.innerHTML = state.customFilters.map((f,i)=>{
          ...
          return `<div class="filter-item flex items-center gap-2 p-2 rounded">
            <span class="flex-1">${d.label} <span class="${col==='g'?'g':col==='r'?'r':'y'} font-semibold">${f.operator} ${q(f.value)}${unit}</span></span>
        ...
        listBox.querySelectorAll('.remove-filter').forEach(b=>{
          b.addEventListener('click', e=>{
            const idx=+e.currentTarget.dataset.i; state.customFilters.splice(idx,1); renderList(); schedule();
          });
        });
      }
      renderList();
    ...
    function passCustomFilters(arr){
      ...
      return arr.filter(e=>{
        for(const f of state.customFilters){
          ...
          let ok=true;
          switch(f.operator){ case'>=':ok=v>=x;break; case'>':ok=v>x;break; case'=':ok=v===x;break;
            case'<':ok=v<x;break; case'<=':ok=v<=x;break; case'!=':ok=v!==x;break; }
          if(!ok) return false;
        }
        return true;
      });
    ...
    // ---------- sélection de métriques (mirroring à droite) ----------
    Object.keys(METRICS).forEach(metric=>{
      ...
      cb.addEventListener('change',(e)=>{
        ...
        schedule(); // → la droite se met à jour avec TOUTES les métriques cochées
      });
    });
    ...
    function calculate(){
      ...
      if(state.mode==='balanced'){
        const scores = arr.map(etf=>{
          let s=0,k=0; sel.forEach(m=>{ const v=METRICS[m].get(etf); if(!Number.isFinite(v)) return;
            const {min,max}=ranges[m]; let z=(max===min)?0.5:(v-min)/(max-min); if(!METRICS[m].max) z=1-z; s+=z; k++; });
        ...
        const idx = arr.map((_,i)=>i).sort((ia,ib)=>{
          for(const m of sel){ let pa=ranks[m][ia], pb=ranks[m][ib];
            if(!Number.isFinite(pa)&&!Number.isFinite(pb)) continue;
            if(!Number.isFinite(pa)) return 1; if(!Number.isFinite(pb)) return -1;
            if(!METRICS[m].max){ pa=1-pa; pb=1-pb; }
            const d=pa-pb; if(Math.abs(d)>0.01) return d>0?-1:1;
          } return 0;
        });
    ...
    function render(entries){
      ...
      entries.forEach((entry,i)=>{
        ...
        const metricsHTML = state.selectedMetrics.map(m=>{
          ...
          const raw=d.get(e); if(!Number.isFinite(raw)) return '';
          let val;
          if(m==='aum'){ const M=raw; val = (M>=1000)? (M/1000).toFixed(1)+'B$' : Math.round(M)+'M$'; }
          else if(d.unit==='%'){ val = fmt(raw,2)+'%'; } else { val = fmt(raw,2); }
          let cls='g';
          if(m==='ter') cls = raw<0.2?'g':raw<0.4?'g':'y';
          else if(m==='volatility') cls = raw<10?'g':raw<20?'g':'y';
          else if(m==='sharpe_proxy') cls = raw>2?'g':raw>1?'g':'y';
          else cls = raw>=0 ? 'g':'r';
          return `<div class="metric"><div class="k">${d.label}</div><div class="v ${cls}">${val}</div></div>`;
        }).join('');

        const typeBadge = classify(e)==='bonds' ? '<span class="badge">Obligations</span>' :
                          classify(e)==='commodity' ? '<span class="badge">Matières</span>' :
    ...
    document.getElementById('etf-mc-reset')?.addEventListener('click',()=>{
      state.mode='balanced';
      state.selectedMetrics=['return_ytd','dividend_yield','volatility','sharpe_proxy','aum','ter','return_1y','return_1d'];
      ...
      document.getElementById('etf-custom-filters-list')?.replaceChildren(
        Object.assign(document.createElement('div'),{className:'text-xs opacity-50 text-center py-2',innerText:'Aucun filtre personnalisé'})
      );
      calculate();
    });
