...
document.addEventListener('DOMContentLoaded', function() {
    ...
    (function () {
        ...
        const timer = setInterval(() => {
            ...
            const hasRows = document.querySelector('[id$="-indices-body"] tr:not(.details-row)');
            if (input && hasRows) {
                input.value = q;
    ...
    function normalizeRecord(r, fallbackRegion) {
        ...
        return {
            ...
            // v9.2: champs pour le barème sectoriel (banque=ROE, utility/REIT=dividende)
            roe_avg_3y: r.roe_avg_3y ?? null,
    ...
    // computeDurability : SOURCE UNIQUE dans js/durability.js (chargé avant ce script → window.computeDurability)
    ...
    function computeIndustryContext(stock) {
        ...
        const medians = {};
        for (const k of ['pe_ratio', 'roe', 'de_ratio', 'fcf_yield', 'net_margin', 'dividend_yield_ttm', 'payout_ratio_ttm', 'beta', 'volatility_3y', 'roic_avg_3y', 'perf_1m', 'perf_3m', 'perf_1y', 'ytd', 'max_drawdown_3y']) {
            const vals = peers.map(s => numf(s[k])).filter(v => v != null);
    ...
    function durabilityPhrase(dur, ic, stock) {
        if (!dur || dur.insufficient || dur.score == null) return '';
        const nm = (stock.name || stock.ticker || '').replace(/\s+(SE|S\.?A\.?|N\.?V\.?|PLC|AG|Inc\.?|Corp\.?|Co\.?|Ltd\.?|Class [A-Z].*)$/gi, '').replace(/[,]+$/, '').trim();
        const groups = [...new Set(dur.crit.map(c => c.group))];
        const strong = [], weak = [];
        for (const g of groups) {
            const cs = dur.crit.filter(c => c.group === g);
            const mn = Math.min(...cs.map(c => c.val));
            if (mn >= 0.75) strong.push(g.toLowerCase()); else if (mn <= 0.25) weak.push(g.toLowerCase());
        }
        const head = { A: 'business solide', B: 'business correct', C: 'business à surveiller', D: 'business fragile' }[dur.grade] || 'business';
        const andJoin = a => a.length <= 1 ? (a[0] || '') : a.slice(0, -1).join(', ') + ' et ' + a[a.length - 1];
        const parts = [];
        // 1) qui c'est + verdict + forces/faiblesses
        let p1 = `${nm} : ${head} (durabilité ${dur.grade})`;
        if (strong.length) p1 += `, porté par ${andJoin(strong)}`;
        if (weak.length) p1 += (strong.length ? ', mais freiné par ' : ', freiné par ') + andJoin(weak);
        parts.push(p1);
        // 2) positionnement CHIFFRÉ dans son industrie + meilleure/pire dimension
        if (ic) {
            const pos = (ic.durRank && ic.durOf)
                ? (ic.durRank === 1 ? `n°1 de ${ic.industry} en durabilité` : ic.durRank <= 3 ? `dans le top 3 de ${ic.industry}` : `#${ic.durRank}/${ic.durOf} de ${ic.industry} en durabilité`)
                : `dans ${ic.industry}`;
            const byPct = ic.ranks.map(r => ({ ...r, pct: r.rank / r.of })).sort((a, b) => a.pct - b.pct);
            const best = byPct[0], worst = byPct[byPct.length - 1];
            let dim = best ? `meilleur sur ${best.l} (#${best.rank}/${best.of})` : '';
            if (worst && worst !== best && worst.pct > 0.5) dim += `${dim ? ', ' : ''}à la traîne sur ${worst.l} (#${worst.rank}/${worst.of})`;
            parts.push(`${pos}${dim ? ` — ${dim}` : ''}`);
        }
        // 3) alertes
        if (dur.mirage) parts.push('⚠ le grade « vs pairs » le flatte, la durabilité est en dessous');
        if (dur.profile === 'finance') parts.push('angle mort : le risque crédit / capital n\'est pas visible');
        return parts.join('. ') + '.';
    }
                                ...
                                <td colspan="10" style="background:rgba(0,255,135,0.02); border-top: 1px solid var(--card-border);">
                                    ...
                                    <div class="grid md:grid-cols-3 gap-6 p-4">
                                        <div>
                                            ...
                                            <div class="space-y-1 text-sm">
                                                <div><span class="opacity-60">1 mois:</span> <span class="${stock.perf_1m && stock.perf_1m.includes('-') ? 'negative' : 'positive'}">${stock.perf_1m||'–'}</span>${_med('perf_1m','%')}</div>
                                                <div><span class="opacity-60">3 mois:</span> <span class="${stock.perf_3m && stock.perf_3m.includes('-') ? 'negative' : 'positive'}">${stock.perf_3m||'–'}</span>${_med('perf_3m','%')}</div>
                                                <div><span class="opacity-60">1 an:</span> <span class="${perf1yClass}">${stock.perf_1y||'–'}</span>${_med('perf_1y','%')}</div>
                                                <div><span class="opacity-60">YTD:</span> <span class="${ytdClass}">${stock.ytd||'–'}</span>${_med('ytd','%')}</div>
                                            </div>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <div class="space-y-1 text-sm">
                                                ...
                                                <div><span class="opacity-60">Div TTM:</span> ${stock.dividend_yield_ttm || stock.dividend_yield || '–'}${_med('dividend_yield_ttm','%')}</div>
                                                <div><span class="opacity-60">Payout:</span> <span class="${stock.payout_class}">${stock.payout_ratio || '–'}</span>${_med('payout_ratio_ttm','%')}</div>
                                            </div>
                                            ...
                                            <div class="space-y-1 text-sm">
                                                ...
                                                <div><span class="opacity-60">Drawdown max 3Y:</span> <span class="negative">${stock.max_drawdown_3y || '–'}</span>${_med('max_drawdown_3y','%')}</div>
