/**
 * partage-succession.js v1.2 — Fix state refs + wire loyerMensuel → rendement + affectif
 *
 * v1.1: Rendement/loyers dans simulation partage
 * v1.2: Fix state.immos→state.immo, state.financials→state.finance
 *       Compute rendement from loyerMensuel * 12 / valeur
 *       Pass affectif flag from immo items
 *
...
const PartageSuccession = (function() {
    ...
    function simulerPartage(params) {
        ...
        heritiers.forEach(function(h) {
            ...
            partsLegales.push({ nom: h.nom, lien: h.lien, partLegale: frac, montantDu: Math.round(masseNette * frac),
                revenuDu: Math.round(revenuTotal * frac), souhaite: h.souhaite || [], attributionPref: !!h.attributionPref });
        });
        ...
        if (conjoint && hasConjoint) {
            ...
            if (rpBien) {
                attributionPref = { beneficiaire: conjoint.nom, bien: rpBien.nom, valeur: rpBien.valeur,
                    dePleinDroit: unionType === 'mariage', soulteAPayer: Math.max(0, rpBien.valeur - conjoint.montantDu),
                    delaiPaiement: unionType === 'mariage' ? '10 ans' : 'Immédiat', article: 'Art. 831-2 CC' };
            }
        }
        ...
        return { applicable: true, massePartageable: masseTotal, dettes: dettes, masseNette: masseNette,
            ...
            partsLegales: partsLegales, lots: lots, attributionPreferentielle: attributionPref,
    ...
    function composerLots(biens, heritiers, attrPref) {
        ...
        var biensDisp = biens.map(function(b) {
            return { nom: b.nom, valeur: b.valeur, type: b.type, divisible: b.divisible !== false,
                ...
                revenuAnnuel: b._revenuAnnuel || 0 };
        });
        ...
        restants.forEach(function(bien) {
            ...
            if (meilleur) { meilleur.biens.push(bien); meilleur.valeurLot += bien.valeur; meilleur.revenuLot += bien.revenuAnnuel; bien.attribue = true; }
        });
    ...
    function calculerEquite(lots, heritiers, masseNette, revenuTotal) {
        ...
        return { score: score, scoreValeur: scoreValeur, scoreRevenu: scoreRevenu, niveau: niveau, couleur: couleur,
            ecartMax: ecartMaxVal, soulteMaximale: ecartMaxVal, ecartMaxRevenu: ecartMaxRev,
            interpretation: score >= 90 ? 'Partage quasi parfait.' : score >= 70 ? 'Partage équilibré.' : score >= 50 ? 'Soultes significatives.' : 'Très déséquilibré — conflit probable.' };
    }
        ...
        lots.forEach(function(lot) {
            if (lot.montantDu > 0 && lot.valeurLot < lot.montantDu * 0.75)
                lesions.push({ heritier: lot.heritier, montantDu: lot.montantDu, valeurRecue: lot.valeurLot,
                    deficit: lot.montantDu - lot.valeurLot, deficitPct: Math.round((1 - lot.valeurLot / lot.montantDu) * 100), article: 'Art. 889 CC' });
        });
        return { lesionDetectee: lesions.length > 0, cas: lesions,
            warning: lesions.length > 0 ? '\ud83d\udea8 LÉSION : ' + lesions.length + ' héritier(s) < 75%. Annulable (art. 889 CC, 2 ans).' : '' };
    }
    ...
    function genererAlertes(biens, lots, heritiers, equite, lesion, revenuTotal) {
        ...
        if (indivisibles.length > 0 && heritiers.length > 1)
            alertes.push({ type: 'warning', icon: 'fa-puzzle-piece', message: indivisibles.length + ' bien(s) indivisible(s) : ' + indivisibles.map(function(b) { return b.nom; }).join(', '),
                conseil: 'Un héritier prend le bien + soulte, ou licitation.' });
        ...
        if (maxSoulte > 50000) alertes.push({ type: 'warning', icon: 'fa-euro-sign', message: 'Soulte maximale : ' + fmt(maxSoulte), conseil: 'Conjoint : délai 10 ans. Autres : immédiat.' });
        if (lesion.lesionDetectee) alertes.push({ type: 'error', icon: 'fa-gavel', message: lesion.warning, conseil: 'Rééquilibrer les lots.' });
        if (revenuTotal > 0 && lots.length >= 2) {
            var rendements = lots.map(function(l) { return l.rendementLot || 0; });
            var maxRdt = Math.max.apply(null, rendements), minRdt = Math.min.apply(null, rendements);
            if (maxRdt - minRdt > 2) alertes.push({ type: 'warning', icon: 'fa-chart-line', message: 'Rendement déséquilibré (écart ' + Math.round((maxRdt - minRdt) * 10) / 10 + ' pts)',
                conseil: 'Mélanger biens à fort et faible rendement par lot.' });
        }
        return alertes;
    }
    ...
    function proposerAlternatives(biens, lots, equite, lesion) {
        ...
        if (equite.score < 70 || lesion.lesionDetectee)
            alternatives.push({ id: 'vente_partielle', label: 'Vente de certains biens + partage du produit', adapte: equite.score < 50 });
    ...
    function renderPartagePanel() {
        ...
        var biens = [];
        // FIX v1.2: state.immo (pas immos) + calcul rendement depuis loyerMensuel
        (state.immo || []).forEach(function(b) {
            var loyer = b.loyerMensuel || 0;
            var valeur = b.valeur || 0;
            var rendement = valeur > 0 ? loyer * 12 / valeur : 0;
            biens.push({ nom: b.label || b.nom || 'Bien immobilier', valeur: valeur, type: 'immo', divisible: false,
                isRP: b.usageActuel === 'rp', affectif: !!b.affectif, rendement: rendement });
        });
        // FIX v1.2: state.finance (pas financials)
        (state.finance || []).forEach(function(f) {
            if (f.type !== 'assurance_vie' && f.type !== 'per')
                biens.push({ nom: f.nom || f.type, valeur: f.montant || 0, type: 'financier', divisible: true, rendement: 0 });
        });
        // Pro assets
        (state.pro || []).forEach(function(p) {
            if (p.valeur > 0)
                biens.push({ nom: p.nom || 'Actif pro', valeur: Math.round(p.valeur * (p.pctDetention || 100) / 100), type: 'pro', divisible: false, rendement: 0 });
        });
        ...
        html += '<strong>Conseil : Donation-partage DE SON VIVANT</strong> \u2014 R\u00e9partir maintenant \u00e9vite tout conflit. (Art. 1076+ CC)</div></div>';
    ...
    function init() {
        ...
        console.log('[PartageSuccession v1.2] Loaded \u2014 fix state refs + rendement loyers + affectif');
    }
