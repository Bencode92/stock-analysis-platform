 ...
 *
 * 1. Composition des lots : répartit les biens en lots équitables
 * 2. Détection de lésion (> 25% → annulation possible, art. 889 CC)
 * 3. Calcul des soultes (compensation financière)
 * 4. Attribution préférentielle (conjoint = logement, art. 831-2 CC)
 * 5. Indice d'équité du partage (score 0-100)
 * 6. Alertes : biens indivisibles, conflits potentiels, blocages
 * 7. Alternatives : vente partielle, maintien indivision, convention
 *
...
const PartageSuccession = (function() {
    ...
    // ============================================================
    // 1. SIMULER LE PARTAGE DES BIENS
    // ============================================================
    ...
    function simulerPartage(params) {
        ...
        if (biens.length === 0 || heritiers.length === 0) {
            return { applicable: false, raison: 'Pas de biens ou pas d\'héritiers' };
        }
        ...
        heritiers.forEach(function(h) {
            var partValeur = Math.round(masseNette * (h.partLegale || (1 / heritiers.length)));
            partsLegales.push({
                nom: h.nom, lien: h.lien, partLegale: h.partLegale || (1 / heritiers.length),
                montantDu: partValeur, souhaite: h.souhaite || [], attributionPref: !!h.attributionPref
            });
        });

        // Attribution préférentielle conjoint (logement)
        ...
        if (conjoint && hasConjoint) {
            ...
            if (rpBien) {
                attributionPref = {
                    ...
                    warnings: rpBien.valeur > conjoint.montantDu * 2
                        ? ['Soulte très élevée — conjoint devra emprunter ou liquider des actifs']
                        : []
                };
            }
        }
        ...
        var lesion = detecterLesion(lots, partsLegales);
    ...
    function composerLots(biens, heritiers, attrPref) {
        ...
        heritiers.forEach(function(h) {
            ...
            if (attrPref && attrPref.beneficiaire === h.nom) {
                ...
                if (rpIdx >= 0) {
                    lot.biens.push(biensDisp[rpIdx]);
                    lot.valeurLot += biensDisp[rpIdx].valeur;
                    biensDisp[rpIdx].attribue = true;
                }
            }
            ...
            if (h.souhaite && h.souhaite.length > 0) {
                h.souhaite.forEach(function(souhait) {
                    ...
                    if (idx >= 0 && lot.valeurLot + biensDisp[idx].valeur <= h.montantDu * 1.30) {
                        lot.biens.push(biensDisp[idx]);
                        lot.valeurLot += biensDisp[idx].valeur;
                        biensDisp[idx].attribue = true;
                    }
                });
            }
            lots.push(lot);
        ...
        // Répartir restants (plus gros d'abord → héritier le plus en dessous)
    ...
    function calculerEquite(lots, heritiers, masseNette) {
        ...
        return {
            ...
            ecartMax: ecartMax, ecartMoyen: Math.round(ecartMoyen), soulteMaximale: ecartMax,
            interpretation: score >= 90 ? 'Partage quasi parfait — écarts minimes.'
                ...
                : 'Partage très déséquilibré — risque de conflit élevé. Vente ou convention recommandée.'
        };
    }

    // ============================================================
    // 4. DÉTECTION LÉSION (> 25%, art. 889 CC)
    // ============================================================
    ...
    function detecterLesion(lots) {
        ...
        lots.forEach(function(lot) {
            if (lot.montantDu > 0 && lot.valeurLot < lot.montantDu * 0.75) {
                var deficitPct = Math.round((1 - lot.valeurLot / lot.montantDu) * 100);
                lesions.push({
                    heritier: lot.heritier, montantDu: lot.montantDu, valeurRecue: lot.valeurLot,
                    deficit: lot.montantDu - lot.valeurLot, deficitPct: deficitPct, annulable: true,
                    article: 'Art. 889 CC', delaiAction: '2 ans',
                    explanation: lot.heritier + ' lésé de ' + deficitPct + '% (> 25%). Partage annulable.'
                });
            }
        });
        return {
            lesionDetectee: lesions.length > 0, cas: lesions,
            warning: lesions.length > 0
                ? '\ud83d\udea8 LÉSION : ' + lesions.length + ' héritier(s) < 75% de leur part. Annulation possible (art. 889 CC, 2 ans).'
                : 'Aucune lésion.'
        };
    }
    ...
    function genererAlertes(biens, lots, heritiers, equite, lesion) {
        ...
        if (maxSoulte > 50000) {
            alertes.push({ type: 'warning', icon: 'fa-euro-sign',
                message: 'Soulte maximale : ' + fmt(maxSoulte),
        ...
        if (lesion.lesionDetectee) {
            alertes.push({ type: 'error', icon: 'fa-gavel', message: lesion.warning,
                conseil: 'Rééquilibrer les lots. Risque annulation judiciaire (art. 889 CC, délai 2 ans).' });
        }
        ...
        if (nbImmo >= 2 && heritiers.length >= 2) {
            ...
            if (pctImmo > 70) {
                alertes.push({ type: 'info', icon: 'fa-building',
                    ...
                    conseil: 'Envisager la vente d\'un bien locatif pour financer les soultes.' });
            }
        }
        return alertes;
    }
    ...
    function proposerAlternatives(biens, lots, equite, lesion) {
        ...
        if (equite.score < 70 || lesion.lesionDetectee) {
            alternatives.push({ id: 'vente_partielle', label: 'Vente de certains biens + partage du produit',
                avantage: 'Partage parfaitement équitable en valeur', inconvenient: 'Perte patrimoine + PV', adapte: equite.score < 50 });
        }
    ...
    function renderPartagePanel() {
        ...
        (state.immos || []).forEach(function(b) {
            biens.push({ nom: b.nom || 'Bien immobilier', valeur: b.valeur || 0, type: 'immo', divisible: false, isRP: b.usage === 'rp' || b.isRP });
        });
        (state.financials || []).forEach(function(f) {
            if (f.type !== 'assurance_vie' && f.type !== 'per') {
                biens.push({ nom: f.nom || f.type, valeur: f.montant || 0, type: 'financier', divisible: true });
            }
        });

        var heritiers = [], bens = state.beneficiaries || [];
        bens.forEach(function(b) {
            heritiers.push({ nom: b.nom || b.lien, lien: b.lien, partLegale: 1 / bens.length, attributionPref: b.lien === 'conjoint_pacs' });
        });
        ...
        ['Héritier', 'Part légale', 'Biens reçus', 'Valeur lot', 'Soulte'].forEach(function(h) {
            html += '<th style="padding:10px 12px;text-align:' + (h === 'Héritier' ? 'left' : 'right') + ';font-weight:600;">' + h + '</th>';
        });
        html += '</tr></thead><tbody>';
        result.lots.forEach(function(lot) {
            ...
            html += '<td style="padding:8px 12px;font-weight:600;">' + lot.heritier + '</td>';
        ...
        html += '<strong>Conseil : Donation-partage DE SON VIVANT</strong> — Répartir maintenant évite tout conflit au décès. Valeurs figées, paix familiale. (Art. 1076+ CC)</div>';
        html += '</div>';
