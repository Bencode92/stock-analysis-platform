...
// Version 3.7 - Mai 2025 - Ajout du sélecteur de type d'activité pour micro-entreprise
...
function addCustomStyles() {
    ...
    styleElement.textContent = `
        ...
        /* Style pour le sélecteur de type d'activité micro */
        .micro-type-badge {
            padding: 0.2rem 0.5rem;
            border-radius: 4px;
            font-size: 0.7rem;
        ...
        .micro-type-badge.vente {
            background-color: rgba(16, 185, 129, 0.2);
            color: #10B981;
        }
        
        .micro-type-badge.service {
            background-color: rgba(59, 130, 246, 0.2);
            color: #60A5FA;
        }
        
        .micro-type-badge.bnc {
            background-color: rgba(139, 92, 246, 0.2);
            color: #A78BFA;
        }
    `;
    ...
    if (formContainer) {
        ...
        optionsRow.innerHTML = `
            <div class=\"bg-blue-900 bg-opacity-30 p-4 rounded-lg\">
                ...
                <!-- Type d'activité pour Micro-entreprise -->
                <div class=\"mb-4\">
        ...
        // Ajouter un événement aux cases à cocher et au select de type micro
...
function runComparison() {
    ...
    // Récupérer le type de micro-entreprise
    const typeMicro = document.getElementById('micro-type') ? document.getElementById('micro-type').value : 'BIC_SERVICE';
    ...
    console.log(\"Type Micro-entreprise:\", typeMicro);
    ...
    // Badge type micro-entreprise
    const microTypeBadges = {
        'BIC_VENTE': '<span class=\"micro-type-badge vente\">Vente (71%)</span>',
        'BIC_SERVICE': '<span class=\"micro-type-badge service\">Service (50%)</span>',
        'BNC': '<span class=\"micro-type-badge bnc\">BNC (34%)</span>'
    };
    ...
    const statutsComplets = {
        'micro': { 
            ...
            simuler: () => window.SimulationsFiscales.simulerMicroEntreprise({
                ca: ca,
                typeMicro: typeMicro, // Utiliser le type sélectionné
                tmiActuel: tmi,
                modeExpert: modeExpert
            })
        },
        ...
        if (statutsComplets[statutId]) {
            try {
                ...
                resultats.push({
                    ...
                    typeMicro: statutId === 'micro' ? typeMicro : null
                });
            } catch (e) {
    ...
    resultats.forEach((res, index) => {
        ...
        // Ajouter badge pour le type de micro-entreprise si applicable
        let microBadge = '';
        if (res.statutId === 'micro' && res.typeMicro && microTypeBadges[res.typeMicro]) {
            microBadge = microTypeBadges[res.typeMicro];
        }
        ...
        row.innerHTML = `
            <td class=\"px-4 py-3 font-medium\">
                ...
                ${statutIcons[res.statutId] || ''} ${res.statut} ${regimeBadges[res.statutId] || ''} ${microBadge}
            </td>
...
function showCalculationDetails(statutId, simulationResults) {
    ...
    if (statutId === 'micro') {
        // Déterminer l'abattement selon le type de micro-entreprise
        let abattementRate;
        let typeLabel;
        
        switch(result.sim.typeMicro) {
            case 'BIC_VENTE':
                abattementRate = '71%';
                typeLabel = 'BIC Vente';
                break;
            ...
            default:
                abattementRate = '50%';
                typeLabel = 'BIC Services';
        }
        
        detailContent = `
            <h2 class=\"text-2xl font-bold text-green-400 mb-4\">Détail du calcul - Micro-entreprise (${typeLabel})</h2>
