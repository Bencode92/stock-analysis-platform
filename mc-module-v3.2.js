...
// v3.2: TTM s'affiche uniquement quand coché (comportement cohérent avec autres métriques)
(function(){
  ...
  // MÉTRIQUES v3.2 avec REG et TTM
  const METRICS = {
    // Performance
  ...
  // Synchroniser checkboxes v3.2 avec REG/TTM
  ...
  // RENDU v3.2 - TTM visible uniquement si sélectionné
  function render(entries){
    ...
    top.forEach((e,i)=>{
      ...
      // v3.2: SUPPRESSION du bloc d'affichage automatique de TTM
      // Le TTM ne s'affiche désormais QUE s'il est coché
      ...
      // Badge "incl. spé" conditionnel (visible seulement si TTM est sélectionné)
      const reg = getDivReg(e.s);
      const ttm = getDivTTM(e.s);
      let specialBadge = '';
      
      // Ne montrer le badge que si TTM est sélectionné pour cohérence
      const showBadge = state.selectedMetrics.includes('dividend_yield_ttm');
      if (showBadge && Number.isFinite(reg) && Number.isFinite(ttm) && ttm - reg >= 0.5) {
  ...
  loadData().then(() => {
    console.log('✅ MC Module v3.2 - TTM visible uniquement si sélectionné');
