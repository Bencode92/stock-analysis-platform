/**
 * fiscal-comparison-enhanced-debug.js
 * VERSION DEBUG avec logs détaillés pour tracer le problème des régimes identiques
 * 
 * LOGS AJOUTÉS:
 * ✅ Début de calculateRegime() - voir quel régime est calculé
 * ✅ Default case amélioré - éviter le fallback silencieux
 * ✅ isRegimeApplicable() - voir quels régimes sont exclus
 * ✅ compareAllRegimes() - tracer chaque simulation
 */
...
class FiscalComparator {
    ...
    async compareAllRegimes(data) {
        console.log('🚀 DÉBUT compareAllRegimes avec data:', data);
        ...
        for (const regime of this.regimes) {
            console.log('🔍 Simulation pour régime:', regime.id);
        ...
        console.log('📊 Résultats finaux triés:', results.map(r => ({
            regime: r.nom,
            cashflow: r.cashflowNetAnnuel
        })));
    ...
    isRegimeApplicable(regime, data, baseResults) {
        ...
        if (conditions.loyerAnnuelMax && loyerAnnuel > conditions.loyerAnnuelMax) {
            console.log(`❌ ${regime.id} NON applicable - loyer annuel ${loyerAnnuel}€ > max ${conditions.loyerAnnuelMax}€`);
            return false;
        }
        
        if (conditions.loyerAnnuelMin && loyerAnnuel < conditions.loyerAnnuelMin) {
            console.log(`❌ ${regime.id} NON applicable - loyer annuel ${loyerAnnuel}€ < min ${conditions.loyerAnnuelMin}€`);
            return false;
        }
        
        // Pour cette version simplifiée, on considère tous les régimes applicables
        console.log(`✅ ${regime.id} est applicable.`);
        return true;
    }
    ...
    calculateRegime(regime, baseResults, data) {
        console.log(`🧪 Calcul du régime: ${regime.id} (type: ${regime.calcul?.type || 'aucun'})`);
        ...
        switch (calcul.type) {
            case 'micro':
                console.log(`➡️ Calcul micro pour ${regime.id}`);
            ...
            case 'reel':
                console.log(`➡️ Calcul réel pour ${regime.id}`);
                result = this.calculateNuReel(result, baseResults, data, commonData);
                break;
                
            case 'reel-amortissement':
                console.log(`➡️ Calcul réel avec amortissement pour ${regime.id}`);
                result = this.calculateLMNPReel(result, baseResults, data, commonData);
                break;
                
            case 'professionnel':
                console.log(`➡️ Calcul professionnel pour ${regime.id}`);
                result = this.calculateLMP(result, baseResults, data, commonData);
                break;
                
            case 'societe':
                console.log(`➡️ Calcul société pour ${regime.id}`);
                result = this.calculateSCIIS(result, baseResults, data, commonData);
                break;
                
            default:
                console.warn(`❌ Type de régime inconnu ou manquant pour ${regime.id} (type: ${calcul.type})`);
                return {
                    ...result,
                    erreur: `Régime non reconnu (${regime.id})`,
                    cashflowNetAnnuel: 0,
                    cashflowMensuel: 0,
                    rendementNet: 0,
                    impotAnnuel: 0
        ...
        console.log(`✅ Résultat pour ${regime.id}:`, {
            cashflow: result.cashflowNetAnnuel,
            impot: result.impotAnnuel
        });
    ...
    calculateMicroFoncier(result, baseResults, data, calcul) {
        console.log('📊 Calcul Micro-foncier - DÉBUT');
    ...
    calculateNuReel(result, baseResults, data, calcul) {
        console.log('📊 Calcul Location nue au réel - DÉBUT');
    ...
    calculateLMNPMicroBIC(result, baseResults, data, calcul) {
        console.log('📊 Calcul LMNP Micro-BIC - DÉBUT');
    ...
    calculateSCIIS(result, baseResults, data, calcul) {
        console.log('📊 Calcul SCI à l\'IS - DÉBUT');
