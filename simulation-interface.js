...
function formatTableValueV2(value, config) {
    if (config.isPercentage) return value.toFixed(2) + '%';
    if (config.unit === 'm²')    return value.toFixed(1) + ' m²';
    if (config.unit === '/mois') return formatNumberV2(value) + ' €/mois';
    ...
    try {
        const tbody = COMPARISON_TABLE_CONFIG.map(section => {
            ...
            const rowsHtml = section.rows.map(row => {
                ...
                let valClassique = row.calculate ? row.calculate(classique)
                                                 : classique[classiqueKey] ?? 0;
                let valEncheres  = row.calculate ? row.calculate(encheres)
            ...
            if (section.totalRow) {
                ...
                const valClassique = total.calculate ? total.calculate(classique)
                                                     : classique[total.key] ?? 0;
                const valEncheres  = total.calculate ? total.calculate(encheres)
