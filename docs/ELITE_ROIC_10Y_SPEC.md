...
## 22. RÉPONSES EXPERT DU 16/09 (Q1–Q6) — appliquées

- **Q1 Diagnostic §15 = pipeline** : `portfolio_engine/elite_diagnostic.py` → `data/elite_diagnostic.json` (CI, après le socle). Fiche descriptive par tenu à ≤ −20 % sur 1 an ou sur la liste de surveillance (Adobe, Genpact, Reply, Publicis) : CA / marge / ROIC dernier exercice vs 3 précédents, médiane d'industrie, porte de sortie la plus proche de casser (distance en %). Verdict « comptes : tiennent / se dégradent ». **Aucun effet sur la sélection.** 16/09 : 11 fiches, 10 « tiennent », Mueller « ROIC 33 vs 41 » (à relire après correction du split).
- **Q2 Journal** : *« nous détenons des franchises en re-notation tant que leurs comptes tiennent ; nous sortirons après le marché, jamais avant, et nous acceptons ce coût. »* Le test n'est pas le cours d'Adobe/Genpact/Reply/Publicis, c'est : si leurs comptes se dégradent, la clé (Trajectoire, durabilité, ROIC < 8) doit réagir. Sinon = défaut de méthode.
- **Q3 Exposition au maillon (pilier 3)** : la preuve chiffrée s'applique aux maillons nommés. **≥ 50 % du CA → plein ; 20-50 % → demi-poids (le reste va au budget en attente) ; < 20 % → pas un maillon.** Champ `maillon_share_pct` (+ source, + `estimated`) sur la fiche société du framework ; absent → plein mais « à documenter avant 2026-12-14 ». Valeurs posées le 16/09 = estimations segments RA 2025 (source à joindre) : TE, Amphenol, ASML, Cadence, KLA, TSM 100 · Schneider 78 · Thales 52 · ABB 45 · Nexans 33 · Safran 20 · Cummins 19 · Caterpillar 15. **Résultat : 11 lignes / 38,1 % / cher 5,8 %** — Caterpillar et Cummins sortent ; ABB, Nexans, Safran à demi-poids ; IA-infra = TE + Amphenol (9,2 %, partiel).
- **Q4 Trajectoire « ROIC ≥ 10 OU marge ↑ »** : permissive pour un cyclique, gardée (gel). Chantier 2027 : évaluer les deux branches sur moyennes 3 ans.
- **Q5 Compteur ≤ 5 / ≤ 10** : démarre en décembre. Budget de transition daté (fin mars 2027) : socle 10 (15/09) + 3 (16/09) = 13 ; conviction 5 in / 6 out (16/09) + 2 out (16/09 Q3). En régime, deux vagues à deux jours d'écart ne seraient jamais acceptables.
- **Q6 Le cash du pilier 3 = « budget en attente de prix »** : reste dans le bloc, ne remonte pas au pilier 2, pas de compte à rebours ni de redistribution ; il entre quand Vertiv, GEV, Cameco passent la porte d'absurdité. Écrit dans portfolios.json (`_bloc_actions`) : **75 socle / 9,5 conviction investie / 15,5 en attente de prix**.
- **Notes** : Mueller à 3,94 % = artefact du sector-balanced (un secteur à 1 nom pèse comme un secteur à 8) → chantier 2027, rien avant. Look-through tous piliers : viser < 25 % par secteur quand l'agrégation sera écrite (le pilier 2 ajoute Nasdaq + semi).

## 23. PORTE 0 COMPLÉTÉE — Milan et places nordiques inaccessibles (Benoit, 2026-09-16 midi)

Trading 212 ne sert ni Borsa Italiana ni Stockholm / Copenhague / Helsinki / Oslo. `INACCESSIBLE_MICS = {XMIL, XSTO, XCSE, XHEL, XOSL}` dans les deux modules (un titre italien servi via Xetra reste achetable). 160 cotations écartées, pool socle 260.
- **Socle** : sortent Technogym, Reply (Milan), Generic Sweden (Stockholm) ; entrent **IDEXX, Jack Henry, Ferrari (NYSE)**. 40 lignes, **US 31 / Europe 9** — drapeau « revue > 70 % sur une région » levé (informatif, pas une porte). 3 changements ; total construction socle : 10 + 3 + 3 = 16.
- **Conviction** : +nVent (② refroidissement liquide, 2,33× après recalcul des médianes, exposition datacenter ≈ 25 % → demi-poids 2,3 %). **12 lignes / 40,4 % / cher 6,2 %.** Bloc : 75 / 10,1 / 14,9.
- **Bug prix corrigé (8229f28aa)** : 136 lignes Europe (Suède, Italie, Norvège…) portaient les cours d'un homonyme américain — le dernier recours `{symbol}` nu du pipeline prix renvoyait la cotation US (GENI = Genius Sports NYSE, TD = Toronto-Dominion, SNX = TD Synnex, PHO = un ETF). Garde d'entité `tdMarketOk` sur tous les appels prix/stats : réponse US pour un titre non-US = rejetée. Cache scorer purgé (136). Résiduel 14 lignes (OTC/PINX, ELON…) — hors périmètre de toute façon. **Generic Sweden était dans le socle avec les prix, la volatilité et le drawdown de Genius Sports** : sa fiche « −47 % 1 an, vol 57 % » du diagnostic §15 était celle d'une autre société.
